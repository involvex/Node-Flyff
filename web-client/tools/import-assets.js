...
const packName =
  argv[1] ||
  path.basename(zipPath, path.extname(zipPath)).replace(/[^a-z0-9_-]/gi, "_");
...
const files = fs
  .readdirSync(outDir)
  .filter((f) => f.match(/\.(png|jpg|jpeg|svg|webp)$/i));
for (const f of files) {
  const part = detectPart(f);
  manifest.assets.avatar[part] = manifest.assets.avatar[part] || [];
  const rel = `/assets/${packName}/${f}`;
  if (!manifest.assets.avatar[part].includes(rel))
    manifest.assets.avatar[part].push(rel);
}
