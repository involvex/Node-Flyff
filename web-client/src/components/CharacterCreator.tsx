...
import { encodeCertify, parseWelcome, parseServerList, encodeSelectServer, parseAck } from "../net/flyffPacket";
...
export default function CharacterCreator() {
  ...
  const [wsAdapter] = useState(() => new WebSocketAdapter("ws://localhost:8080"));
        ...
        <div className="cc-grid">
          <div className="cc-form">
            ...
            <div className="cc-attr">
              <div>
                ...
                <div className="attr-controls">
                  <button onClick={() => setFace((f) => Math.max(0, f - 1))}>{"<"}</button>
              ...
              <div>
                ...
                <div className="attr-controls">
                  <button onClick={() => setHair((h) => Math.max(0, h - 1))}>{"<"}</button>
              ...
              <div>
                ...
                <div className="attr-controls">
                  <button onClick={() => setHairColor((c) => Math.max(0, c - 1))}>{"<"}</button>
                  <div className="attr-value">{hairColor}</div>
                  <button onClick={() => setHairColor((c) => c + 1)}>{">"}</button>
                </div>
              </div>
            </div>
            ...
            <div className="start-row">
              <button className="cc-start" onClick={handleStart}>Start</button>
            </div>
          </div>
          ...
          <div className="cc-preview">
            <div className="preview-box">
              <div className="preview-canvas-wrap">
                <AvatarPreview gender={genders[genderIdx]} face={face} hair={hair} hairColor={hairColor} width={140} height={140} />
              </div>
            ...
            {servers
              ...
              <div className="server-list">
                ...
                <ul>
                  {servers.map((s) => (
                    <li key={s} className="server-item" onClick={() => handleSelectServer(s)}>{s}</li>
                  ))}
                </ul>
              </div>
                )
