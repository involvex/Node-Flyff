...
fs.readFile(
  (() => {
    ...
    return path.join(__dirname, "../data", "character-school.inc");
  })(),
  "utf16le",
  (err, data) => {
    if (err) {
    ...
    fs.writeFile(
      path.join(__dirname, "../custom", "characterSchool.yaml"),
      yamlData,
      (err) => {
        if (err) {
          console.error(err);
        ...
        console.log("Conversion completed. Output written to characterSchool.yaml");
      }
    );
  }
);
