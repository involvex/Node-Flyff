...
fs.readFile(path.join(__dirname, "../data", "world.inc"), "utf16le", (
  err,
  data
) => {
  if (err) {
  ...
  lines.forEach((line) => {
    if (line === "(" || line === ")" || line.startsWith("//") || !line.trim()) {
      return;
    }
    const parts = line
      .trim()
  ...
  fs.writeFile(path.join(__dirname, "../custom", "world.yaml"), yamlData, (
    err
  ) => {
    if (err) {
      console.error(err);
