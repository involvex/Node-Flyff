...
import { JobMax, JobType } from "../../../common/defineJob";

const __filename = fileURLToPath(import.meta.url);
const __dirname = dirname(__filename);

// Read the file content
fs.readFile(path.join(__dirname, "../data", "propJob.inc"), "utf8", (
  err,
  data
) => {
  if (err) {
  ...
  fs.writeFile(path.join(__dirname, "../custom", "job.yaml"), yamlData, (
    err
  ) => {
    if (err) {
      console.error(err);
