...
export class SqliteClient implements IRedisClient {
  ...
  async insertCluster(cluster: ICluster): Promise<void> {
    this.db.prepare(`INSERT OR REPLACE INTO clusters (name, host, port, lastPing, enabled, channels) VALUES (?, ?, ?, ?, ?, ?)`)
      .run(cluster.name, cluster.host, cluster.port, cluster.lastPing || 0, cluster.enabled ? 1 : 0, JSON.stringify(cluster.channels || []));
  }
  ...
  async insertChannel(clusterName: string, channel: IChannel): Promise<void> {
    const cluster = (await this.getCluster(clusterName)) || { name: clusterName, host: "127.0.0.1", port: 0, lastPing: 0, channels: [], enabled: true } as ICluster;
    if (_.some(cluster.channels, (c) => c.name === channel.name)) return;
    cluster.channels.push(channel);
    await this.insertCluster(cluster);
  }

  async updateChannel(clusterName: string, updatedChannel: IChannel): Promise<void> {
    const cluster = await this.getCluster(clusterName);
    if (!cluster) return;
    const idx = _.findIndex(cluster.channels, { name: updatedChannel.name });
    if (idx >= 0) cluster.channels[idx] = { ...cluster.channels[idx], ...updatedChannel };
    else cluster.channels.push(updatedChannel);
    await this.insertCluster(cluster);
  }

  async getChannel(clusterName: string, channelName: string): Promise<IChannel | null> {
    const cluster = await this.getCluster(clusterName);
  ...
  async deleteChannel(clusterName: string, channelName: string): Promise<void> {
    const cluster = await this.getCluster(clusterName);
    if (!cluster) return;
    cluster.channels = _.filter(cluster.channels, (c) => c.name !== channelName);
    await this.insertCluster(cluster);
  }
  ...
  async setCharacterSession(sessionKey: number, characterId: number, username: string, password: string, expireInSeconds: number): Promise<void> {
    ...
    this.db.prepare(`INSERT OR REPLACE INTO sessions (sessionKey, characterId, username, password, expireAt) VALUES (?, ?, ?, ?, ?)`)
  ...
  async getCharacterSession(sessionKey: number): Promise<{ characterId: number; username: string; password: string } | null> {
    ...
    return { characterId: row.characterId, username: row.username, password: row.password };
  }
