...
import { WeaponKindType, getWeaponKindType } from "../../../common/weaponKindType";
...
export class AttackArbiterBase {
  ...
  getWeaponAttackPower(entity: Mover, weapon: Item | null): RangeHelper<number> {
    ...
    return new RangeHelper<number>(
      Math.round((entity.attributes.get(DefineAttributes.DST_ABILITY_MIN) + abilityMin) * multiplier) + power,
      Math.round((entity.attributes.get(DefineAttributes.DST_ABILITY_MAX) + abilityMax) * multiplier) + power
    );
  }
  ...
  getWeaponItemMultiplier(weapon: Item | null): number {
    ...
    const refine = getWeaponKindType(referStat) === WeaponKindType.Ultimate ? Item.WeaponArmorRefineMax : (weapon?.refine ?? 0);
