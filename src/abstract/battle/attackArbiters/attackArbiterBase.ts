...
export class AttackArbiterBase {
    ...
    switch (weaponType) {
      case WeaponType.MELEE_SWD:
        attribute =
          ...
          player.attributes.get(DefineAttributes.DST_STR) -
          12;
      ...
      case WeaponType.RANGE_BOW:
        attribute =
          (player.statistics.dexterity +
            player.attributes.get(DefineAttributes.DST_DEX) -
            14) *
          4;
        levelFactor = player.level * 1.3;
        jobFactor =
  ...
  getWeaponAttackPower(
    ...
    const power = refine > 0 ? Math.pow(refine, 1.5) : 0;

    const abilityMin = (weapon?.properties?.dwAbilityMin ?? 0) as number;
    const abilityMax = (weapon?.properties?.dwAbilityMax ?? 0) as number;
  ...
  getWeaponItemMultiplier(weapon: Item | null): number {
    ...
    let multiplier = 1.0;
    const referStat = weapon?.properties?.dwReferStat1 ?? 0;
    const refine =
      getWeaponKindType(referStat) === WeaponKindType.Ultimate
        ? Item.WeaponArmorRefineMax
        : (weapon?.refine ?? 0);
  ...
  getWeaponExtraDamages(entity: Mover, weapon: Item | null): number {
    ...
    switch (weapon.properties?.dwWeaponType) {
      case WeaponType.MELEE_SWD:
        extraDamages =
          entity.attributes.get(DefineAttributes.DST_SWD_DMG) +
          entity.attributes.get(DefineAttributes.DST_TWOHANDMASTER_DMG);
        break;
      case WeaponType.MELEE_AXE:
        extraDamages =
          entity.attributes.get(DefineAttributes.DST_AXE_DMG) +
          entity.attributes.get(DefineAttributes.DST_TWOHANDMASTER_DMG);
        break;
      case WeaponType.KNUCKLE:
        extraDamages =
          entity.attributes.get(DefineAttributes.DST_KNUCKLE_DMG) +
          entity.attributes.get(DefineAttributes.DST_KNUCKLEMASTER_DMG);
        break;
      case WeaponType.MELEE_YOYO:
        extraDamages =
          entity.attributes.get(DefineAttributes.DST_YOY_DMG) +
          entity.attributes.get(DefineAttributes.DST_YOYOMASTER_DMG);
        break;
      case WeaponType.RANGE_BOW:
        extraDamages =
          entity.attributes.get(DefineAttributes.DST_BOW_DMG) +
          entity.attributes.get(DefineAttributes.DST_BOWMASTER_DMG);
        break;
    }
    ...
    if (this.attacker instanceof Player) {
      ...
      const attribute =
        this.defender instanceof Player
          ? DefineAttributes.DST_PVP_DMG
          : DefineAttributes.DST_MONSTER_DMG;
